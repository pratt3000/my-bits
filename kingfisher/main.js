  ...
  async init(ctx) {
    ...
    const waterMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: [
        ...
        "  float a = dot(vW.xz, vec2(2.1, 1.7)) + time * 1.6;",
        "  float b = dot(vW.xz, vec2(-1.3, 2.6)) + time * 1.1;",
        "  float c = dot(vW.xz, vec2(3.7, -2.9)) - time * 2.3;",
        "  vec2 g = 0.045 * cos(a) * vec2(2.1, 1.7) + 0.03 * cos(b) * vec2(-1.3, 2.6) + 0.012 * cos(c) * vec2(3.7, -2.9);",
        "  vec3 N = normalize(vec3(-g.x, 1.0, -g.y));",
        "  float fres = pow(1.0 - max(0.0, dot(N, V)), 2.6);",
        "  vec3 col = mix(deep, shallow, 0.1 + 0.9 * fres);",
        "  vec3 H = normalize(V + sunD);",
        "  float spec = pow(max(0.0, dot(N, H)), 420.0);",
        "  float glit = smoothstep(0.2, 1.0, sin(vW.x * 19.0 + time * 4.0) * sin(vW.z * 23.0 - time * 3.0));",
        "  col += sunC * spec * (0.5 + 1.8 * glit) * 1.4;",
