...
Touch anywhere to start growing a world — hold longer and it gains mass — then
drag and let go to launch it. A dotted line predicts where it will go while you
aim, so putting something into a closed orbit is a decision rather than a guess.
After release nothing is scripted: every body pulls on every other body, and the
orbits, captures, slingshots and collisions are just what that force law does.
...
- `G = 1400` in world units of CSS pixels, tuned so a planet ~180px out from a
  1600-mass star comes round in about ten seconds — roughly the slowest orbit
  that still reads as motion on a phone.

## Feel

- **Throwing.** The body follows your finger and takes the finger's own velocity
  on release, smoothed, because raw per-event deltas on a phone are noise.
