 ...
 *
 * The case the whole bit is built around is two people slapping in the same
 * instant. bit.tapTogether sends both touch points in a single CDP touchStart,
 * so the bit sees two pointerdown events in the same frame — exactly what two
 * hands landing together look like. Exactly one of them must come out holding
 * the pile, and the other must pay nothing.
 *
...
check(lates.length >= 1, "the other hand was recorded late, not burned (got " + lates.length + ")");
...
for (let i = 0; i < 200; i++) {
  const s = await rs();
  if (!s || s.phase === "over" || s.phase === "resolve") break;
  if (s.slap) {
    // Whoever is not locked takes it, so the piles actually move around.
    const taker = s.counts.find((c) => !c.out && c.lock === 0) || s.counts.find((c) => !c.out);
    await bit.tap(pads[taker.seat].x, pads[taker.seat].y);
    await bit.wait(60);
    continue;
  }
  if (!shotMid && s.pile >= 4) { await bit.shot("ratscrew-7-midgame"); shotMid = true; }
  if (!(await flip())) { await bit.wait(120); }
}
